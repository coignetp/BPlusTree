...
TEST (NodeAddItem, SimpleSplit) {
  ...
  ASSERT_EQ(children.begin()->first, 0);
  ASSERT_EQ(children.begin()->second->Length(), 2);
  ASSERT_EQ(children.begin()->second->GetThisItem(1), 1);
  ASSERT_EQ(std::next(children.begin())->first, 2);
  ASSERT_EQ(std::next(children.begin())->second->Length(), 4);
  ASSERT_EQ(std::next(children.begin())->second->GetThisItem(1), 3);
}
...
TEST (NodeAddItem, MultipleSplit) {
  ...
  ASSERT_EQ(node.GetChildren().begin()->first, 0);
  ASSERT_EQ(node.GetThisChild(1)->Length(), 5);
  ASSERT_EQ(std::next(node.GetChildren().begin(), 1)->first, 6);
...
TEST (NodeDeleteItem, NodeDelete) {
  ...
  ASSERT_EQ(node.GetThisChild(0)->Length(), 3);
  ASSERT_EQ(node.GetChildren().begin()->first, 1);
